using System;
using Com.Aspose.Barcode.Api;
using Com.Aspose.Barcode.Model;

namespace CSharp.ManagingRecognition.WithoutCloudStorage
{
    class ReadBarcodeFromRequestBody
    {
        public static void Run()
        {
            // ExStart:1
            
            // Instantiate Aspose BarCode Cloud API SDK
            BarcodeApi barcodeApi = new BarcodeApi(Common.APP_KEY, Common.APP_SID, Common.BASEPATH);

            // The barcode type. If this parameter is empty, autodetection of all supported types is used.
            String type = "Code39Standard";

            // Set mode for checksum validation during recognition
            String checksumValidation = "";

            // Set if FNC symbol stripping should be performed
            bool stripFnc = false;

            // Set recognition of rotated barcode
            int rotationAngle = 0;

            // Set request body. Request body can contain raw data bytes of the image or encoded with base64 
            String body = @"
                            9DEyAAAAAElFTkSuQmCC";     

            try
            {
                // Invoke Aspose.BarCode Cloud SDK API to read barcode from request body. Request body can contain raw data bytes of the image or encoded with base64 
                BarcodeResponseList apiResponse = barcodeApi.PostBarcodeRecognizeFromRequestBody(type, checksumValidation, stripFnc, rotationAngle, body);

                if (apiResponse != null)
                {                    
                    Console.WriteLine("Read Barcode from request body, Done!");                    
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine("error:" + ex.Message + "\n" + ex.StackTrace);

            }
            // ExEnd:1
        }
    }
}
