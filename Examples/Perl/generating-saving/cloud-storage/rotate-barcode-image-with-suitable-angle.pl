...
$response = $barcodeApi->PutBarcodeGenerateFile(name => $name, text => $text, type => $type, format => $format, codeLocation => $codeLocation, enableChecksum => $enableChecksum, rotAngle => $rotAngle);
