package AsposeBarCodeCloud::ApiClient;
...
sub new
{
	...
	if(not defined $AsposeBarCodeCloud::Configuration::app_sid or $AsposeBarCodeCloud::Configuration::app_sid eq ''){
		croak("Aspose Cloud App SID key is missing.");
    }
	
	if(not defined $AsposeBarCodeCloud::Configuration::api_key or $AsposeBarCodeCloud::Configuration::api_key eq ''){
    ...
    my (%args) = (
        ...
        'base_url' => $AsposeBarCodeCloud::Configuration::api_server,
        @_
    );
...
sub call_api {
	...
	$resource_path =~ s/\Q{appSid}\E/$AsposeBarCodeCloud::Configuration::app_sid/g;
  
    my $_url = $self->{base_url} . $resource_path;
  
    # update signature
    my $signature = $self->sign($_url, $AsposeBarCodeCloud::Configuration::app_sid, $AsposeBarCodeCloud::Configuration::api_key);
    ...
    $self->{ua}->timeout($self->{http_timeout} || $AsposeBarCodeCloud::Configuration::http_timeout); 
    $self->{ua}->agent($self->{http_user_agent} || $AsposeBarCodeCloud::Configuration::http_user_agent);
